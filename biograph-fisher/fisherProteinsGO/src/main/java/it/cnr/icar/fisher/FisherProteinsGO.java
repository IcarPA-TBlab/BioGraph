...
public class FisherProteinsGO extends AbstractHandler {
    ...
    public static void main(String[] args) throws Exception {
        ...
        server.setHandler(new FisherProteinsGO(client));
        server.start();
        server.join();
    }

    public FisherProteinsGO(Client c) {
    	this.client = c;
    ...
    public void handle(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    	String proteinsList = request.getParameter("proteins");
        ...
        if (proteinsList != null) {
        	String[] proteinsArray = proteinsList.split(",");
        	String opts = "";
        	
        	ArrayList<String> focusProteins = new ArrayList<String>();
        	ArrayList<String> focusAnnotations = new ArrayList<String>();

        	for (int i=0; i<proteinsArray.length; i++) {
        		focusProteins.add(proteinsArray[i]);
        		opts += ".option('" + proteinsArray[i] + "',__.as('" + proteinsArray[i] + "'))";
        	}

    		ResultSet results = client.submit("g.V().hasLabel('Protein').choose(values('name'))" + opts + ".in('ANNOTATES').dedup().values('goId')");
    		...
    		for (String fp : focusAnnotations) {
    			...
    			results = client.submit("g.V().hasLabel('Go').has('goId','" + fp + "').out('ANNOTATES').hasLabel('Protein').choose(values('name'))" + opts + ".values('name').dedup()");
            	results.stream().forEach((Result p) -> { 
